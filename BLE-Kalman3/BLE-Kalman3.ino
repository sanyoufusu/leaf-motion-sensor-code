...
bool connectToServer() {
   ...
   // pClient->setMTU(517); //set client to request maximum MTU from server (default is  23otherwise)
    ...
    pRemoteCharacteristic = pRemoteService->getCharacteristic(charUUID_3);
    if (pRemoteCharacteristic == nullptr) {
      Serial.print("Failed to find our characteristic UUID: ");
      Serial.println(charUUID_3.toString().c_str());
      pClient->disconnect();
      return false;
    }
...
if (doConnect == true) {
  ...
  if (connected) {
    String newValue =/*String("1357P:") + String(-kalAngleY)+String("R:") +*/String(kalAngleX)+String("\t");
